...
class alcoholPredictor(object):
    ...
    def create_dataset(dataset_ENSE, dataset_compatible):
        ...
        dataset_compatible_copy['alcohol'] = w127_values
        dataset_compatible_copy = dataset_compatible_copy.dropna()
        dataset_compatible_copy = dataset_compatible_copy[~dataset_compatible_copy['alcohol'].isin([98,99])]
        dataset_compatible_copy['alcohol'] = dataset_compatible_copy['alcohol'].replace([5,6,7,8,9], 0)    # Menos de 1-2 días a la semana
        dataset_compatible_copy['alcohol'] = dataset_compatible_copy['alcohol'].replace([1,2,3,4], 1)      # Mínimo 1-2 días a la semana
    ...
    def randomforest_predictor(dataset, output_dir):
        ...
        file.write("'alcohol' feature prediction:\n\n")
        ...
        plt.title("Confusion matrix (Dev), 'alcohol' feature")
        plt.tight_layout()
        ...
        print('[RF] Feature importance based on mean decrease in impurity saved in: ' + os.path.join(output_dir, "FeatureImportanceMDI_alcohol.png"))
        plt.close()

        classifier_final = RandomForestClassifier()
        classifier_final.fit(data, labels)

        joblib.dump(classifier_final, os.path.join(output_dir, "RF_MODEL_alcohol.joblib"), compress=3)
    ...
    def dnn_predictor(dataset, output_dir):
        ...
        file.write("DNN model: 'alcohol' feature prediction\n\n")
        ...
        print('[DNN] Trained FINAL model saved in: ' + os.path.join(output_dir, "DNN_MODEL_alcohol.h5"))

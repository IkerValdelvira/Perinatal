...
def insert_peson_semanas(dataset_perinatal, dataset_pesorec):
    pesorec_indices = dataset_pesorec.index.values.tolist()
    peson_values = dataset_perinatal["peson"].loc[pesorec_indices]
    semanas_values = dataset_perinatal["semanas"].loc[pesorec_indices]
    peson_semanas_values = peson_values / semanas_values
    peson_semanas_values = peson_semanas_values.round(decimals=2)
    dataset_pesorec['peson_semanas'] = peson_semanas_values
    dataset_pesorec = dataset_pesorec.dropna()
    return dataset_pesorec
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser( description="Script to perform 'pesorec' classification experiments adding 'peson/semanas' fake feature on 3 created datasets: 'dataPerinatal_remove_items.csv', 'dataPerinatal_predictions.csv' and 'dataPerinatal_remove_features.csv'. Data is splitted in Train(70%), Dev(15%), Test(15%), and multiple oversampling/undersampling techniques can be selected. Usage example: $python experiments_peson_semanas.py dataPerinatal_converted.csv pathTo/Datasets -o pathTo/PesonSemanasExperiments")
    ...
    parser.add_argument("-o", "--output_dir",
                        help="Path to directory for the created datasets with 'peson/semanas' fake feature, Train/Dev/Test sets and results of classification models. Default option: current directory.",
                        default=os.getcwd())
    ...
    print("\nInserting new feature 'peson/semanas' in 'dataPerinatal_remove_items.csv' dataset...")
    dataset_rem_items = insert_peson_semanas(dataset_perinatal, dataset_rem_items)
    print("Saving created dataset in: " + str(os.path.join(output_dir, 'dataPerinatal_remove_items(peson_semanas).csv')))
    dataset_rem_items.to_csv(os.path.join(output_dir, 'dataPerinatal_remove_items(peson_semanas).csv'))

    print("\nInserting new feature 'peson/semanas' in 'dataPerinatal_remove_features.csv' dataset...")
    dataset_rem_features = insert_peson_semanas(dataset_perinatal, dataset_rem_features)
    print("Saving created dataset in: " + str(os.path.join(output_dir, 'dataPerinatal_remove_features(peson_semanas).csv')))
    dataset_rem_features.to_csv(os.path.join(output_dir, 'dataPerinatal_remove_features(peson_semanas).csv'))

    print("\nInserting new feature 'peson/semanas' in 'dataPerinatal_predictions.csv' dataset...")
    dataset_predictions = insert_peson_semanas(dataset_perinatal, dataset_predictions)
    print("Saving created dataset in: " + str(os.path.join(output_dir, 'dataPerinatal_predictions(peson_semanas).csv')))
    dataset_predictions.to_csv(os.path.join(output_dir, 'dataPerinatal_predictions(peson_semanas).csv'))

...
if __name__ == '__main__':
    parser = argparse.ArgumentParser( description="Script to perform 'pesorec' classification using RandomForest and optimized DNN, and with the posibility of feature ablation. Usage example: $python experiments_feature_ablation.py train_predictions.csv dev.csv -a edadm,edadm6,edadm35 -o pathTo/FeatureAblationExperiments")
    parser.add_argument("input_pesorec_train",
                        help="Path to file with input Train dataset for 'pesorec' classification. For example: 'train_predictions.csv'.")
    parser.add_argument("input_pesorec_dev",
                        help="Path to file with input Dev dataset for 'pesorec' classification. For example: 'dev.csv'.")
    parser.add_argument('-a', '--ablation', type=str,
                        help='Features to delete separated by comma. For example: "edadm,edadp". Default: empty')
    parser.add_argument("-o", "--output_dir",
                        help="Path to directory for the created classification models and evaluations.",
                        default=os.getcwd())
    ...
    if ablation:
        ablation = [item for item in ablation.split(',')]
    ...
    if ablation:
        ...
        for feature in ablation:
            ablation_string += feature + "_"
        ablation_string = ablation_string[:-1]
        Path(os.path.join(output_dir, ablation_string)).mkdir(parents=True, exist_ok=True)
        output_dir = os.path.join(output_dir, ablation_string)
        print('\nDeleting following features from TRAIN and DEV sets: ' + str(ablation))
        train_pesorec = train_pesorec.drop(ablation, 1)
        dev_pesorec = dev_pesorec.drop(ablation, 1)
    else:
        Path(os.path.join(output_dir, "AllFeatures")).mkdir(parents=True, exist_ok=True)
        output_dir = os.path.join(output_dir, "AllFeatures")
    ...
    rf_data = experiment_rf(train_pesorec, dev_pesorec, output_dir_aux)
    ...
    dnn_data = experiment_dnn(train_pesorec, dev_pesorec, output_dir_aux)

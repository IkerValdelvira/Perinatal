...
class PaisnxmPredictor(object):

    @staticmethod
    def randomforest_predictor(dataset, output_dir):
        file = open(os.path.join(output_dir, "REPORT_RandomForest_paisnxm.txt"), "w")
        file.write("'paisnxm' feature prediction:\n\n")
        ...
        dataset = dataset.drop(['mimmi', 'fimmi', 'paisnxp', 'estudiom', 'estudiop', 'educm', 'educp', 'conviven'], axis=1)
        ...
        counts = dataset['paisnxm'].value_counts()
        dataset = dataset[~dataset['paisnxm'].isin(counts[counts < 3].index)]

        # Train/Dev split
        labels = dataset['paisnxm']
        data = dataset.drop('paisnxm', axis=1)
        ...
        X_dev_paisnacm = X_dev['paisnacm'].to_numpy()
        ...
        X_train['paisnxm'] = y_train
        X_train = X_train[~X_train['paisnxm'].isin(clases_borrar)]
        X_dev['paisnxm'] = y_dev
        X_dev = X_dev[~X_dev['paisnxm'].isin(clases_borrar)]
        y_train = X_train['paisnxm']
        X_train = X_train.drop('paisnxm', 1)
        y_dev = X_dev['paisnxm']
        X_dev = X_dev.drop('paisnxm', 1)

        # Label encoder: convertir códigos de países a categorías desde 0
        encoder = LabelEncoder()
        encoder.fit(y_train)
        mapping = dict(zip(encoder.classes_, range(len(encoder.classes_))))
        mapping_file = open(os.path.join(output_dir, "label_mapping_paisnxm.json"), "w")
        ...
        # Save the model
        joblib.dump(classifier, os.path.join(output_dir, "RF_MODEL_paisnxm.joblib"), compress=3)
        print('[RF] Trained model saved in: ' + os.path.join(output_dir, "RF_MODEL_paisnxm.joblib"))
        ...
        for i in range(len(X_dev_paisnacm)):
            if (X_dev_paisnacm[i] == y_dev_codes[i]):
                mismo_nac_orig += 1
            if (X_dev_paisnacm[i] == y_pred_dev_codes[i]):
                mismo_nac_orig_pred += 1
            i += 1
        prop_mismo_nac_orig = round(mismo_nac_orig / len(X_dev_paisnacm), 4)
        prop_mismo_nac_orig_pred = round(mismo_nac_orig_pred / len(X_dev_paisnacm), 4)
        file.write("\n\nCoincidences between 'paisnxm' (origin) and 'paisnacm' (nationality) in DEV set:\n")
        file.write("\tSame REAL origin ('paisnxm') and nacionality ('paisnacm'): " + str(prop_mismo_nac_orig) + "%\n")
        file.write("\tSame PREDICTED origin ('paisnxm') and nacionality ('paisnacm'): " + str(prop_mismo_nac_orig_pred) + "%\n")
        ...
        plt.title("ROC curve (Dev), 'paisnxm' feature")
        plt.legend(loc="lower right")
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_paisnxm.png"))
        print('[RF] Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_paisnxm.png"))
        plt.close()

        print('[RF] Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_RandomForest_paisnxm.txt"))
        ...
        fig.tight_layout()
        plt.savefig(os.path.join(output_dir, "FeatureImportanceMDI_paisnxm.png"))
        print('[RF] Feature importance based on mean decrease in impurity saved in: ' + os.path.join(output_dir, "FeatureImportanceMDI_paisnxm.png"))
        plt.close()
    ...
    def dnn_predictor(dataset, output_dir):
        ...
        # Borrar variables que se quieren predecir de los datos 1996-2006
        dataset = dataset.drop(['mimmi', 'fimmi', 'paisnxp', 'estudiom', 'estudiop', 'educm', 'educp', 'conviven'],
                               axis=1)
        ...
        X_train = X_train[~X_train['paisnxm'].isin(clases_borrar)]
        X_val['paisnxm'] = y_val
        X_val = X_val[~X_val['paisnxm'].isin(clases_borrar)]
        X_dev['paisnxm'] = y_dev
        X_dev = X_dev[~X_dev['paisnxm'].isin(clases_borrar)]
        y_train = X_train['paisnxm']
        X_train = X_train.drop('paisnxm', 1)
        y_val = X_val['paisnxm']
        X_val = X_val.drop('paisnxm', 1)
        y_dev = X_dev['paisnxm']
        X_dev = X_dev.drop('paisnxm', 1)
        ...
        joblib.dump(scaler, os.path.join(output_dir, 'std_scaler_paisnxm.bin'), compress=True)
        print("[DNN] Standard scaler saved in: " + str(os.path.join(output_dir, 'std_scaler_paisnxm.bin')))
        ...
        file = open(os.path.join(output_dir, "REPORT_DNN_paisnxm.txt"), "w")
        file.write("DNN model: 'paisnxm' feature prediction\n\n")
        ...
        print('[DNN] Trained model saved in: ' + os.path.join(output_dir, "DNN_MODEL_paisnxm.h5"))
        model.save(os.path.join(output_dir, "DNN_MODEL_paisnxm.h5"))
        ...
        mismo_nac_orig_pred = 0
        for i in range(len(X_dev_paisnacm)):
            if (X_dev_paisnacm[i] == y_real_dev_codes[i]):
                mismo_nac_orig += 1
            if (X_dev_paisnacm[i] == y_pred_dev_codes[i]):
                mismo_nac_orig_pred += 1
            i += 1
        ...
        file.write("\n\nCoincidences between 'paisnxm' (origin) and 'paisnacm' (nationality) in DEV set:\n")
        file.write("\tSame REAL origin ('paisnxm') and nacionality ('paisnacm'): " + str(prop_mismo_nac_orig) + "%\n")
        file.write("\tSame PREDICTED origin ('paisnxm') and nacionality ('paisnacm'): " + str(
        ...
        plt.legend(loc="lower right")
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_paisnxm.png"))
        print('[DNN] Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_paisnxm.png"))
        plt.close()

        print('[DNN] Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_DNN_paisnxm.txt"))
    ...
    def model_comparison(rf_data, dnn_data, output_dir):
        ...
        plt.title("ROC curve (Dev), 'paisnxm' feature")
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurveComparison(Dev)_paisnxm.png"))
        print('\n[Comparison RF/DNN] Dev ROC curve and AUC comparison saved in: ' + os.path.join(output_dir, "ROCcurveComparison(Dev)_paisnxm.png"))
        plt.close()

...
class MimmiPredictor(object):

    @staticmethod
    def randomforest_predictor(dataset, output_dir):
        file = open(os.path.join(output_dir, "REPORT_RandomForest_mimmi.txt"), "w")
        file.write("'mimmi' feature prediction:\n\n")
        ...
        dataset = dataset.drop(['fimmi', 'paisnxm', 'paisnxp', 'estudiom', 'estudiop', 'educm', 'educp', 'conviven'], axis=1)

        # Borrar items con missing values
        dataset = dataset.dropna()

        # Train/Dev split
        labels = dataset['mimmi']
        data = dataset.drop('mimmi', axis=1)
        ...
        X_dev_mforeign = X_dev['mforeign'].to_numpy()
        ...
        # Save the model
        joblib.dump(classifier, os.path.join(output_dir, "RF_MODEL_mimmi.joblib"), compress=3)
        print('[RF] Trained model saved in: ' + os.path.join(output_dir, "RF_MODEL_mimmi.joblib"))
        ...
        plt.title("Confusion matrix (Dev), 'mimmi' feature")
        sn.heatmap(df_cm_dev, annot=True, fmt='g', cmap=plt.cm.Blues)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_mimmi.png"))
        plt.close()
        print('[RF] Confusion matrix saved in: ' + os.path.join(output_dir, "ConfusionMatrix_mimmi.png"))
        ...
        # Buscar coincidencias entre mimmi (origen) y mforeign (nacionalidad) en Test
        mismo_nac_orig = 0
        mismo_nac_orig_pred = 0
        for i in range(len(X_dev_mforeign)):
            if (X_dev_mforeign[i] == y_dev.iloc[i]):
                mismo_nac_orig += 1
            if (X_dev_mforeign[i] == y_pred_dev[i]):
                mismo_nac_orig_pred += 1
            i += 1
        prop_mismo_nac_orig = round(mismo_nac_orig / len(X_dev_mforeign), 4)
        prop_mismo_nac_orig_pred = round(mismo_nac_orig_pred / len(X_dev_mforeign), 4)
        file.write("\n\nCoincidences between 'mimmi' (origin) and 'mforeign' (nationality) in DEV set:\n")
        file.write("\tSame REAL origin ('mimmi') and nacionality ('mforeign'): " + str(prop_mismo_nac_orig) + "%\n")
        file.write("\tSame PREDICTED origin ('mimmi') and nacionality ('mforeign'): " + str(prop_mismo_nac_orig_pred) + "%\n")
        ...
        # title
        plt.title("ROC curve (Dev), 'mimmi' feature")
        # x label
        plt.xlabel('False Positive Rate')
        # y label
        plt.ylabel('True Positive rate')
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_mimmi.png"))
        print('[RF] Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_mimmi.png"))
        plt.close()

        print('[RF] Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_RandomForest_mimmi.txt"))
        ...
        fig.tight_layout()
        plt.savefig(os.path.join(output_dir, "FeatureImportanceMDI_mimmi.png"))
        print('[RF] Feature importance based on mean decrease in impurity saved in: ' + os.path.join(output_dir, "FeatureImportanceMDI_mimmi.png"))
        plt.close()
    ...
    def dnn_predictor(dataset, output_dir):
        ...
        joblib.dump(scaler, os.path.join(output_dir, 'std_scaler_mimmi.bin'), compress=True)
        print("[DNN] Standard scaler saved in: " + str(os.path.join(output_dir, 'std_scaler_mimmi.bin')))
        ...
        file = open(os.path.join(output_dir, "REPORT_DNN_mimmi.txt"), "w")
        file.write("DNN model: 'mimmi' feature prediction\n\n")
        ...
        print('[DNN] Trained model saved in: ' + os.path.join(output_dir, "DNN_MODEL_mimmi.h5"))
        model.save(os.path.join(output_dir, "DNN_MODEL_mimmi.h5"))
        ...
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_mimmi.png"))
        plt.close()
        print('[DNN] Confusion matrix saved in: ' + os.path.join(output_dir, "ConfusionMatrix_mimmi.png"))
        ...
        for i in range(len(X_dev_mforeign)):
            if (X_dev_mforeign[i] == y_dev.iloc[i]):
                mismo_nac_orig += 1
            if (X_dev_mforeign[i] == predicted_classes_dev[i][0]):
                mismo_nac_orig_pred+= 1
            i += 1
        prop_mismo_nac_orig = round(mismo_nac_orig / len(X_dev_mforeign), 4)
        prop_mismo_nac_orig_pred = round(mismo_nac_orig_pred / len(X_dev_mforeign), 4)
        ...
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_mimmi.png"))
        print('[DNN] Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_mimmi.png"))
        plt.close()

        print('[DNN] Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_DNN_mimmi.txt"))
    ...
    def model_comparison(rf_data, dnn_data, output_dir):
        ...
        plt.title("ROC curve (Dev), 'mimmi' feature")
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurveComparison(Dev)_mimmi.png"))
        print('\n[Comparison RF/DNN] Dev ROC curve and AUC comparison saved in: ' + os.path.join(output_dir, "ROCcurveComparison(Dev)_mimmi.png"))
        plt.close()
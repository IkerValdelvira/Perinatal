...
class FimmiPredictor(object):

    @staticmethod
    def randomforest_predictor(dataset, output_dir):
        file = open(os.path.join(output_dir, "REPORT_RandomForest_fimmi.txt"), "w")
        file.write("'fimmi' feature prediction:\n\n")
        ...
        dataset = dataset.drop(['mimmi', 'paisnxm', 'paisnxp', 'estudiom', 'estudiop', 'educm', 'educp', 'conviven'], axis=1)

        # Borrar items con missing values
        dataset = dataset.dropna()

        # Train/Dev split
        labels = dataset['fimmi']
        data = dataset.drop('fimmi', axis=1)
        ...
        X_train, X_dev, y_train, y_dev, indices_train, indices_dev = train_test_split(data, labels, indices,
                                                                                                 ...
                                                                                                 shuffle=True,
                                                                                                 stratify=labels)
        X_dev_fforeign = X_dev['fforeign'].to_numpy()
        ...
        # Save the model
        joblib.dump(classifier, os.path.join(output_dir, "RF_MODEL_fimmi.joblib"), compress=3)
        print('[RF] Trained model saved in: ' + os.path.join(output_dir, "RF_MODEL_fimmi.joblib"))
        ...
        plt.title("Confusion matrix (Dev), 'fimmi' feature")
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_fimmi.png"))
        plt.close()
        print('[RF] Confusion matrix saved in: ' + os.path.join(output_dir, "ConfusionMatrix_fimmi.png"))
        ...
        # Buscar coincidencias entre fimmi (origen) y fforeign (nacionalidad) en Test
        mismo_nac_orig = 0
        mismo_nac_orig_pred = 0
        for i in range(len(X_dev_fforeign)):
            if (X_dev_fforeign[i] == y_dev.iloc[i]):
                mismo_nac_orig += 1
            if (X_dev_fforeign[i] == y_pred_dev[i]):
                mismo_nac_orig_pred += 1
            i += 1
        prop_mismo_nac_orig = round(mismo_nac_orig / len(X_dev_fforeign), 4)
        prop_mismo_nac_orig_pred = round(mismo_nac_orig_pred / len(X_dev_fforeign), 4)
        file.write("\n\nCoincidences between 'fimmi' (origin) and 'fforeign' (nationality) in DEV set:\n")
        file.write("\tSame REAL origin ('fimmi') and nacionality ('fforeign'): " + str(prop_mismo_nac_orig) + "%\n")
        file.write("\tSame PREDICTED origin ('fimmi') and nacionality ('fforeign'): " + str(prop_mismo_nac_orig_pred) + "%\n")
        ...
        # title
        plt.title("ROC curve (Dev), 'fimmi' feature")
        # x label
        plt.xlabel('False Positive Rate')
        # y label
        plt.ylabel('True Positive rate')
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_fimmi.png"))
        print('[RF] Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_fimmi.png"))
        plt.close()

        print('[RF] Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_RandomForest_fimmi.txt"))
        ...
        fig.tight_layout()
        plt.savefig(os.path.join(output_dir, "FeatureImportanceMDI_fimmi.png"))
        print('[RF] Feature importance based on mean decrease in impurity saved in: ' + os.path.join(output_dir, "FeatureImportanceMDI_fimmi.png"))
        plt.close()
    ...
    def dnn_predictor(dataset, output_dir):
        ...
        joblib.dump(scaler, os.path.join(output_dir, 'std_scaler_fimmi.bin'), compress=True)
        print("[DNN] Standard scaler saved in: " + str(os.path.join(output_dir, 'std_scaler_fimmi.bin')))
        ...
        file = open(os.path.join(output_dir, "REPORT_DNN_fimmi.txt"), "w")
        file.write("DNN model: 'fimmi' feature prediction\n\n")
        ...
        print('[DNN] Trained model saved in: ' + os.path.join(output_dir, "DNN_MODEL_fimmi.h5"))
        model.save(os.path.join(output_dir, "DNN_MODEL_fimmi.h5"))
        ...
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_fimmi.png"))
        plt.close()
        print('[DNN] Confusion matrix saved in: ' + os.path.join(output_dir, "ConfusionMatrix_fimmi.png"))

        # Ded predictions probabilities
        i = 0
        results = []
        for y in y_dev:
            success = "YES"
        ...
        for i in range(len(X_dev_fforeign)):
            if (X_dev_fforeign[i] == y_dev.iloc[i]):
                mismo_nac_orig += 1
            if (X_dev_fforeign[i] == predicted_classes_dev[i][0]):
                mismo_nac_orig_pred += 1
            i += 1
        prop_mismo_nac_orig = round(mismo_nac_orig / len(X_dev_fforeign), 4)
        prop_mismo_nac_orig_pred = round(mismo_nac_orig_pred / len(X_dev_fforeign), 4)
        ...
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_fimmi.png"))
        print('[DNN] Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_fimmi.png"))
        plt.close()

        print('[DNN] Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_DNN_fimmi.txt"))
    ...
    def model_comparison(rf_data, dnn_data, output_dir):
        ...
        plt.plot(rf_data['fpr_dev'], rf_data['tpr_dev'], color='red', label='RandomForest, ROC curve (AUC = {:.4f})'.format(rf_data['auc_dev']))
        plt.plot(dnn_data['fpr_dev'], dnn_data['tpr_dev'], color='green', label='DNN, ROC curve (AUC = {:.4f})'.format(dnn_data['auc_dev']))
        plt.xlabel('False positive rate')
        plt.ylabel('True positive rate')
        plt.title("ROC curve (Dev), 'fimmi' feature")
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurveComparison(Dev)_fimmi.png"))
        print('\n[Comparison RF/DNN] Dev ROC curve and AUC comparison saved in: ' + os.path.join(output_dir, "ROCcurveComparison(Dev)_fimmi.png"))
        plt.close()
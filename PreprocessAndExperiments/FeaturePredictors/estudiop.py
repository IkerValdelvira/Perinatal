...
class EstudiopPredictor(object):

    @staticmethod
    def randomforest_predictor(dataset, output_dir):
        file = open(os.path.join(output_dir, "REPORT_RandomForest_estudiop.txt"), "w")
        file.write("'estudiop' feature prediction:\n\n")
        ...
        dataset = dataset.drop(['mimmi', 'fimmi', 'paisnxm', 'paisnxp', 'estudiom', 'educm', 'educp', 'conviven'], axis=1)

        # Borrar items con missing values
        dataset = dataset.dropna()

        # Train/Dev split
        labels = dataset['estudiop']
        labels = labels - 1  # Etiquetas: 0-11 (no 1-12)
        data = dataset.drop('estudiop', axis=1)
        ...
        # Save the model
        joblib.dump(classifier, os.path.join(output_dir, "RF_MODEL_estudiop.joblib"), compress=3)
        print('[RF] Trained model saved in: ' + os.path.join(output_dir, "RF_MODEL_estudiop.joblib"))
        ...
        plt.title("Confusion matrix (Dev), 'estudiop' feature")
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_estudiop.png"))
        plt.close()
        print('[RF] Confusion matrix saved in: ' + os.path.join(output_dir, "ConfusionMatrix_estudiop.png"))
        ...
        plt.title("ROC curve (Dev), 'estudiop' feature")
        plt.legend(loc="lower right")
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_estudiop.png"))
        print('[RF] Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_estudiop.png"))
        plt.close()

        print('[RF] Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_RandomForest_estudiop.txt"))
        ...
        fig.tight_layout()
        plt.savefig(os.path.join(output_dir, "FeatureImportanceMDI_estudiop.png"))
        print('[RF] Feature importance based on mean decrease in impurity saved in: ' + os.path.join(output_dir, "FeatureImportanceMDI_estudiop.png"))
        plt.close()
    ...
    def dnn_predictor(dataset, output_dir):
        ...
        joblib.dump(scaler, os.path.join(output_dir, 'std_scaler_estudiop.bin'), compress=True)
        print("[DNN] Standard scaler saved in: " + str(os.path.join(output_dir, 'std_scaler_estudiop.bin')))
        ...
        file = open(os.path.join(output_dir, "REPORT_DNN_estudiop.txt"), "w")
        file.write("DNN model: 'estudiop' feature prediction\n\n")
        ...
        print('[DNN] Trained model saved in: ' + os.path.join(output_dir, "DNN_MODEL_estudiop.h5"))
        model.save(os.path.join(output_dir, "DNN_MODEL_estudiop.h5"))
        ...
        plt.savefig(os.path.join(output_dir, "ConfusionMatrix_estudiop.png"))
        plt.close()
        print('[DNN] Confusion matrix saved in: ' + os.path.join(output_dir, "ConfusionMatrix_estudiop.png"))
        ...
        plt.legend(loc="lower right")
        plt.savefig(os.path.join(output_dir, "ROCcurve(Dev)_estudiop.png"))
        print('[DNN] Dev ROC curve and AUC saved in: ' + os.path.join(output_dir, "ROCcurve(Dev)_estudiop.png"))
        plt.close()

        print('[DNN] Model evaluation report saved in: ' + os.path.join(output_dir, "REPORT_DNN_estudiop.txt"))
    ...
    def model_comparison(rf_data, dnn_data, output_dir):
        ...
        plt.title("ROC curve (Dev), 'estudiop' feature")
        plt.legend(loc='best')
        plt.savefig(os.path.join(output_dir, "ROCcurveComparison(Dev)_estudiop.png"))
        print('\n[Comparison RF/DNN] Dev ROC curve and AUC comparison saved in: ' + os.path.join(output_dir, "ROCcurveComparison(Dev)_estudiop.png"))
        plt.close()